...
for (let i = 0; i < sizeOfQuestionSet; i++) {
  let question = new Question(
    questionsObject[String(i)]["country"],
    questionsObject[String(i)]["landmark"],
    questionsObject[String(i)]["options"],
    questionsObject[String(i)]["image"],
    questionsObject[String(i)]["description"]
  );
...
function startQuiz() {
  // Capture player's name from the input field
  playerName = document.getElementById("player-name-input").value;
  // Check if the player's name is not empty
  if (playerName.trim() === "") {
    alert("Please enter your name before starting the quiz!");
    return; // Stop the function execution if the name is empty
  }
  ...
  pageContent =
    '<div id="question-container"><div id="timer"><i class="fa-solid fa-clock"></i> ' +
    timeLimitPerQuestion +
    "s</div><h1>Question " +
    currentQuestionNumber +
    '</h1><h2>Where in the world is this?</h2><div id="photo-and-answers"><img class="question-photo" src="images/' +
    questions[currentQuestionNumber - 1].image +
    '"><button class="answer-button">' +
    questions[currentQuestionNumber - 1].options[0] +
    '</button><button class="answer-button">' +
...
function startTimer() {
  ...
  setTimer = setInterval(function () {
    timer.innerHTML =
      `<i class="fa-solid fa-clock"></i> ` + String(secondsRemaining) + `s`;
...
function nextQuestion() {
  ...
  if (currentQuestionNumber <= numQuestionsInQuiz) {
    pageContent =
      '<div id="question-container"><div id="timer"><i class="fa-solid fa-clock"></i> 10s</div><h1>Question ' +
      currentQuestionNumber +
      '</h1><h2>Where in the world is this?</h2><div id="photo-and-answers"><img class="question-photo" src="images/' +
      questions[currentQuestionNumber - 1].image +
      '"><button class="answer-button">' +
    ...
    pageContent =
      '<div id="title-container"><h1 id="your-final-score">' +
      playerName +
      ", your final score is " +
      score +
      " out of " +
      numQuestionsInQuiz +
      '!</h1><h2>Thank you for playing<br><span id="final-page-title">Where in the World?</h2><button id="play-again">Play again!</button></div>';
    //  setTimeout(showQuestion, 500);

    // Display the final score message on the final page
    document.getElementById("main-container").innerHTML = pageContent;
    const finalScoreMessage = document.getElementById("your-final-score");
    ...
    // function to reset the quiz and show the title page
    function resetQuiz() {
      // Clear the previous player name
      playerName = "";
      ...
      // Show the title page content
      const titlePageContent =
        '<div id="title-container"><h1>Welcome to Where in the World?</h1><input type="text" id="player-name-input" placeholder="Enter your name" /><button id="start-button">Start the quiz!</button></div>';
      document.getElementById("main-container").innerHTML = titlePageContent;

      // Re-attach the event listener to the "Start the quiz!" button
      startButton = document.getElementById("start-button");
      startButton.addEventListener("click", startQuiz);
    }
...
function showResult() {
  ...
  // if the wrong answer was chosen:
    ...
    questions[currentQuestionNumber - 1].options.includes(this.innerHTML)
  ) {
  ...
  setTimeout(function () {
    document.getElementById("main-container").innerHTML =
      '<div id="question-container"><h1>' +
      rightOrWrong +
      '</h1><div id="photo-and-answers"><h2>' +
      questions[currentQuestionNumber - 1].landmark +
      ", " +
      questions[currentQuestionNumber - 1].country +
      '</h2><img class="question-photo" src="images/' +
      questions[currentQuestionNumber - 1].image +
      '"><p>' +
      questions[currentQuestionNumber - 1].description +
      '</p><button id="next-question-button">' +
      nextQuestionButtonText +
      ' <i class="fa-solid fa-arrow-right"></i></button></div></div>';
    document
